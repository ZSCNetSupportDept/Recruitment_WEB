...
if ($_POST['change'] == false) {
	...
	$result = file_get_contents("http://wwzx.zsxyww.com/jsonpost.php", false, stream_context_create($options));
} else {
	write_log("\r\n\t修改微信后端数据，返回信息：", __LINE__);
	$result = file_get_contents("http://wwzx.zsxyww.com/jsonmodify.php", false, stream_context_create($options));
}
